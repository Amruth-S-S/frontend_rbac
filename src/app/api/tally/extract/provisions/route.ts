...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TALLY_BASE}/extract/provisions?${params.toString()}`, { headers: { Accept: 'application/json' } });
    ...
    return NextResponse.json({ error: 'Provisions extraction failed' }, { status: 500 });
  }
}
