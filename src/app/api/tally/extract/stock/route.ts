...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TALLY_BASE}/extract/stock?${params.toString()}`, { headers: { Accept: 'application/json' } });
    ...
    return NextResponse.json({ error: 'Stock extraction failed' }, { status: 500 });
  }
}
