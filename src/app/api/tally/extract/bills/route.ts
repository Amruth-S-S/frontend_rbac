...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TALLY_BASE}/extract/bills?${params.toString()}`, { headers: { Accept: 'application/json' } });
    ...
    return NextResponse.json({ error: 'Bills extraction failed' }, { status: 500 });
  }
}
