...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TALLY_BASE}/extract/gst?${params.toString()}`, {
    ...
    const message = isTimeout
      ? 'GST extraction is taking longer than 40 minutes and timed out. The file may be very large — try again or check with the extraction service.'
      : `GST extraction failed: ${err instanceof Error ? err.message : String(err)}`;
    console.error('[extract/gst]', err);
