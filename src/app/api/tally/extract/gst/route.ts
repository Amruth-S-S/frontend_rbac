...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TALLY_BASE}/extract/gst?${params.toString()}`, { headers: { Accept: 'application/json' } });
    ...
    return NextResponse.json({ error: 'GST extraction failed' }, { status: 500 });
  }
}
