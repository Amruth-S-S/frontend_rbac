...
export async function GET(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${TALLY_BASE}/extract/brs?${params.toString()}`, { headers: { Accept: 'application/json' } });
    ...
    return NextResponse.json({ error: 'BRS extraction failed' }, { status: 500 });
  }
}
