...
// GET /api/forecasting/tables/:table?filter_column=&filter_value=&use_date_column=&selected_columns=&date_column=&target_column=
...
export async function GET(req: NextRequest, { params }: { params: Promise<{ table: string }> }) {
  ...
  for (const key of ['filter_column', 'filter_value', 'use_date_column', 'selected_columns', 'date_column', 'target_column']) {
    const v = sp.get(key);
